...
if [ "X${SKIP_TAG_LATEST}" != "Xtrue" ];then
    ...
    if [ "X${DOCKER_REG}" != "X" ];then
      ...
      echo ${BUILD_IMG_REPODIGEST} > ${ARTIFACTS_DIR}/build/${DOCKER_TAG}.image_name
...
if [ "X${DOCKER_TAG_REV}" == "Xtrue" ];then
    ...
    if [ "X${DOCKER_REG}" != "X" ];then
        ...
        echo ${BUILD_IMG_REPODIGEST} > ${ARTIFACTS_DIR}/build/${DOCKER_TAG}-rev${GO_PIPELINE_COUNTER}.image_name
